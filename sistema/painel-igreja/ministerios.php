...
<div class="col-md-12 my-3">
	<a href="#" onclick="inserir()" type="button" class="btn btn-dark btn-sm">Novo Ministério</a>
</div>
		...
		<table id="example" class="table table-striped table-light table-hover my-4 my-4" style="width:100%">
			...
			<tbody>
				...
				for($i=0; $i < $total_reg; $i++){
					...
					//totalizar membros
					$query2 = $pdo->query("SELECT * FROM ministerios_membros where igreja = '$id_igreja' and ministerio = '$id'");
					...
					<tr>
						...
						<td class="esc"><?php echo $nome_secretario ?></td>
						<td class="esc"><?php echo $nome_lider1 ?></td>
						<td class="esc"><?php echo $nome_lider2 ?></td>

						<td>
							<big>
								<a href="#" onclick="editar('<?php echo $id ?>', '<?php echo $nome ?>', '<?php echo $dias ?>', '<?php echo $hora ?>', '<?php echo $pastor ?>', '<?php echo $secretario ?>', '<?php echo $lider1 ?>', '<?php echo $lider2 ?>')" title="Editar Registro">	<i class="bi bi-pencil-square text-primary"></i> </a>

								<a href="#" onclick="excluir('<?php echo $id ?>' , '<?php echo $nome ?>')" title="Excluir Registro">	<i class="bi bi-trash text-danger"></i> </a>

								<a href="#" onclick="dados('<?php echo $nome ?>', '<?php echo $dias ?>', '<?php echo $hora ?>',  '<?php echo $nome_pastor ?>', '<?php echo $nome_secretario ?>', '<?php echo $nome_lider1 ?>', '<?php echo $nome_lider2 ?>', '<?php echo $obs ?>', '<?php echo $membros_celula ?>')" title="Ver Dados">	<i class="bi bi-info-square-fill text-primary"></i> </a>
		...
		<div class="modal-content">
					...
					<div class="row">
						<div class="col-md-4">
							<div class="mb-3">
								...
								<input type="text" class="form-control" id="nome" name="nome" placeholder="Nome do Ministério" required>
							</div>
						</div>
						<div class="col-md-4">
							<div class="mb-3">
					...
					<div class="modal-dialog">
						<div class="modal-content">
							...
							<div class="modal-body">
								...
								<span class=""><b>Secretário:</b> <span id="coordenador-dados"></span></span>
								<hr style="margin:4px">
			...
			<div class="modal fade" id="modalAddMembros" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
				...
				<script type="text/javascript">
					...
					function editar(id, nome, dias, hora, pastor, coordenador, lider1, lider2){
						$('#id').val(id);
						$('#nome').val(nome);
						$('#dias').val(dias);
						...
						$('#secretario').val(coordenador).change();
					...
					function dados(nome, dias, hora, pastor, coordenador, lider1, lider2, obs, membros){
