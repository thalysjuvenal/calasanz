...
if (@$_GET['filtrar'] == "dia") {
	...
	$query = $pdo->query("SELECT * FROM membros where igreja = '$id_igreja' and month(data_nasc) = '$dataMes' and day(data_nasc) = '$dataDia' order by day(data_nasc) asc");

	$query_pastores = $pdo->query("SELECT * FROM coordenadores where igreja = '$id_igreja' and month(data_nasc) = '$dataMes' and day(data_nasc) = '$dataDia' order by day(data_nasc) asc");

} else {
	$classe_mes = 'text-primary';
	$classe_dia = 'text-dark';

	$query = $pdo->query("SELECT * FROM membros where igreja = '$id_igreja' and month(data_nasc) = '$dataMes' order by day(data_nasc) asc");

	$query_pastores = $pdo->query("SELECT * FROM coordenadores where igreja = '$id_igreja' and month(data_nasc) = '$dataMes' order by day(data_nasc) asc");
...
<div class="tabela bg-light">
					...
					<tr class="<?php echo $classe_aniv ?>">
						...
						<td>
							<big>
								...
								<a href="#"
									onclick="dados('<?php echo $nome ?>', '<?php echo $cpf ?>', '<?php echo $email ?>', '<?php echo $telefone ?>', '<?php echo $endereco ?>', '<?php echo $foto ?>', '<?php echo $data_nascF ?>', '<?php echo $data_cadF ?>', '<?php echo $nome_ig ?>', '<?php echo $data_batF ?>', '<?php echo $nome_cargo ?>', '<?php echo $estado ?>')"
									title="Ver Dados"> <i class="bi bi-info-square-fill text-primary"></i> </a>


								<a class="<?php echo $classe_whats ?>" target="_blank"
									href="http://api.whatsapp.com/send?1=pt_BR&phone=55<?php echo $telefone ?>&text=Ola <?php echo $nome ?>, nos da <?php echo $nome_ig ?> desejamos a você um feliz aniversário, que Deus te abençoe e te ilumine.."
					...
					<tr class="<?php echo $classe_aniv ?>">
						...
						<td>
							<big>
								...
								<a href="#"
									onclick="dados('<?php echo $nome ?>', '<?php echo $cpf ?>', '<?php echo $email ?>', '<?php echo $telefone ?>', '<?php echo $endereco ?>', '<?php echo $foto ?>', '<?php echo $data_nascF ?>', '<?php echo $data_cadF ?>', '<?php echo $nome_ig ?>', 'Não Lançado', 'Coordenador')"
									title="Ver Dados"> <i class="bi bi-info-square-fill text-primary"></i> </a>
	...
	<?php } else {
		echo 'Não existem dizimistas aniversáriando este mês';
	} ?>
</div>
