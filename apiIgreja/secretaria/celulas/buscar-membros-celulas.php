...
$query_celula = $pdo->query("SELECT * FROM celulas where id = '$celula'");
$res_celula = $query_celula->fetchAll(PDO::FETCH_ASSOC);
$nome_celula = $res_celula[0]['nome'];

$query = $pdo->prepare("SELECT c.id as cid, c.membro, c.igreja, c.celula, m.id, m.nome from celulas_membros as c INNER JOIN membros as m ON c.membro = m.id where m.nome LIKE '$buscar' and c.celula = '$celula' order by c.id desc");
